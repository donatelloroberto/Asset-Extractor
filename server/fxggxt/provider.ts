...
import { extractFxggxtStreams } from "./extractors.js";
import { FXGGXT_CATALOG_MAP } from "./manifest.js";
...
function parseArticles($: cheerio.CheerioAPI): CatalogItem[] {
  ...
  $("article.loop-video.thumb-block").each((_, el) => {
    ...
    const posterRaw = $el.find(".post-thumbnail-container img").attr("data-lazy-src")
      || $el.find(".post-thumbnail-container img").attr("data-src")
      || $el.find(".post-thumbnail-container img").attr("src");
...
export async function getFxggxtMeta(id: string): Promise<StremioMeta | null> {
  ...
  try {
    ...
    if (!validatedMeta.success) {
      if (isDebug()) console.error(`[Fxggxt] Meta validation error:`, validatedMeta.error.message);
      return null;
    }
