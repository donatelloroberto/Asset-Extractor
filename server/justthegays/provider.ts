...
import { extractJustthegaysStreams } from "./extractors.js";
import { JUSTTHEGAYS_CATALOG_MAP } from "./manifest.js";
...
export async function getJustthegaysMeta(id: string): Promise<StremioMeta | null> {
  ...
  try {
    ...
    if (!validatedMeta.success) {
      if (isDebug()) console.error(`[Justthegays] Meta validation error:`, validatedMeta.error.message);
      return null;
    }
