...
export async function extractBesthdgaypornStreams(pageUrl: string): Promise<ExtractedStream[]> {
  ...
  try {
    ...
    $("video source, video[src], video[data-src]").each((_, el) => {
      ...
      if (src && /\.(mp4|m3u8|webm)/.test(src)) {
        const fullUrl = src.startsWith("http") ? src : (src.startsWith("//") ? `https:${src}` : `https://besthdgayporn.com${src}`);
        addStream(fullUrl);
      }
    });
    ...
    for (const iframeSrc of iframeSrcs) {
      try {
        ...
        if (isDebug()) console.error(`[BestHDgayporn] iframe fetch error for ${iframeSrc}: ${err.message}`);
      }
    }
