...
import { extractGaystreamStreams } from "./extractors.js";
import { GAYSTREAM_CATALOG_MAP } from "./manifest.js";
...
function parseItems($: cheerio.CheerioAPI): CatalogItem[] {
  ...
  $("div.grid-item").each((_, el) => {
    ...
    const posterRaw = $el.find("img.item-img").attr("data-lazy-src")
      || $el.find("img.item-img").attr("data-src")
      || $el.find("img.item-img").attr("src")
      || $el.find("span.item-thumb img").attr("data-lazy-src")
      || $el.find("span.item-thumb img").attr("data-src")
...
export async function getGaystreamMeta(id: string): Promise<StremioMeta | null> {
  ...
  try {
    ...
    if (!validatedMeta.success) {
      if (isDebug()) console.error(`[GayStream] Meta validation error:`, validatedMeta.error.message);
      return null;
    }
