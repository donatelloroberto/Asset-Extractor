...
export async function extractGaystreamStreams(pageUrl: string): Promise<ExtractedStream[]> {
  ...
  try {
    ...
    if (embedUrls.length === 0) {
      ...
      if (iframeSrc) {
        const src = iframeSrc.startsWith("//") ? `https:${iframeSrc}` : iframeSrc;
        embedUrls.push(src);
      }
    }
    ...
    if (downloadLink) {
      streams.push({
        name: "Download",
        url: downloadLink.startsWith("//") ? `https:${downloadLink}` : downloadLink,
        referer: pageUrl,
      });
    }
...
async function resolveEmbed(embedUrl: string, referer: string): Promise<ExtractedStream[]> {
  ...
  if (hostname.includes("bigwarp")) {
    return extractBigwarp(url, referer);
  }

  return extractListMirrorOrGeneric(url, referer);
}
...
async function extractBigwarp(url: string, referer?: string): Promise<ExtractedStream[]> {
  const streams: ExtractedStream[] = [];
  try {
    const html = await fetchPage(url, { referer: referer || url });
    ...
    if (fileMatch) {
      streams.push({ name: "Bigwarp", url: fileMatch[1], referer: url });
    }

    if (streams.length === 0) {
      const srcMatch = html.match(/src\s*:\s*["'](https?:\/\/[^"']+)["']/);
      if (srcMatch) {
        streams.push({ name: "Bigwarp", url: srcMatch[1], referer: url });
      }
    }
  } catch (err: any) {
...
async function extractListMirrorOrGeneric(url: string, referer: string): Promise<ExtractedStream[]> {
  const streams: ExtractedStream[] = [];
  try {
    ...
    if (sourcesMatch) {
      try {
        ...
        for (const source of sources) {
          const mirrorUrl = source.url || source.file || source.src;
          if (mirrorUrl) {
            try {
              const resolved = await resolveEmbed(mirrorUrl, url);
              if (resolved.length > 0) {
                streams.push(...resolved);
              } else {
                streams.push({ name: "Mirror", url: mirrorUrl, referer: url });
              }
            } catch {
              streams.push({ name: "Mirror", url: mirrorUrl, referer: url });
            }
        ...
        if (isDebug()) console.error("[ListMirror] JSON parse error:", err.message);
      }
    }
    ...
    if (streams.length === 0) {
      const fileMatch = html.match(/file\s*:\s*["'](https?:\/\/[^"']+)["']/);
      if (fileMatch) {
        streams.push({ name: new URL(url).hostname, url: fileMatch[1], referer: url });
        return streams;
      }

      const srcMatch = html.match(/src:\s*["'](https?:\/\/[^"']+)["']/);
      if (srcMatch) {
        streams.push({ name: new URL(url).hostname, url: srcMatch[1], referer: url });
      }
    }
  } catch (err: any) {
    if (isDebug()) console.error("[ListMirror/Generic] Extraction error:", err.message);
  }
  return streams;
}
