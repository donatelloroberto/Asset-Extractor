...
function parseArticles($: cheerio.CheerioAPI): CatalogItem[] {
  ...
  $("article.dce-post-item").each((_, el) => {
    ...
    const posterRaw = $el.find(".elementor-widget-theme-post-featured-image img").attr("data-lazy-src")
      || $el.find(".elementor-widget-theme-post-featured-image img").attr("data-src")
      || $el.find(".elementor-widget-theme-post-featured-image img").attr("src");
...
export async function getGaycock4uMeta(id: string): Promise<StremioMeta | null> {
  ...
  try {
    ...
    if (!validatedMeta.success) {
      if (isDebug()) console.error(`[Gaycock4U] Meta validation error:`, validatedMeta.error.message);
      return null;
    }
